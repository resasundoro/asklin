    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">List Data Kategori Persyaratan</h3>
                @can('m-persyaratan-create')
            ...
            <form action="javascript:void(0)" id="form" name="form" class="form-horizontal" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Nama izin persyaratan klinik" id="kategori" name="kategori">
                    </div>
                </div>
...
<script>
    ...
    function add(){
        $('#form').trigger("reset");
        $('.modal-title').html("Tambah Kategori Persyaratan");
        $('#modal').modal('show');
        $('#id').val('');
    }
    ...
    function edit(id){
        $.ajax({
            type:"POST",
            url: '{{ route('m_persyaratan.edit') }}',
            data: { id: id },
            dataType: 'json',
            success: function(data){
                $('.modal-title').html("Edit Kategori Persyaratan");
                $('#modal').modal('show');
                $('#id').val(data.id);
    ...
    function deleteu(id){
        if (confirm("Hapus kategori persyaratan klinik ini?") == true) {
            var id = id;
            $.ajax({
                type:"POST",
                url: "{{ url('m_persyaratan/delete') }}",
                data: { id: id },
                dataType: 'json',
