    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">List Data Kriteria Klinik</h3>
                @can('m-kriteria-klinik-create')
                    <a class="modal-effect btn btn-primary ms-auto" onClick="add()" href="javascript:void(0)">Tambah Kriteria</a>
                @endcan
            </div>
            <div class="card-body">
                    ...
                    <table id="datatable" class="table table-bordered text-nowrap border-bottom">
                        <thead>
                            <tr>
                                ...
                                <th class="border-bottom-0">Kriteria</th>
            ...
            <form action="javascript:void(0)" id="form" name="form" class="form-horizontal" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Nama kriteria klinik" id="kriteria" name="kriteria">
                    </div>
                </div>
...
<script>
    $(function(e) {
        ...
        $('#datatable').DataTable({
            ...
            serverSide: true,
            type: 'JSON',
            ajax: '{{ route('kriteria_klinik.list') }}',
            columns: [
    ...
    function edit(id){
        $.ajax({
            type:"POST",
            url: '{{ route('kriteria_klinik.edit') }}',
            data: { id: id },
            dataType: 'json',
    ...
    $('#form').submit(function(e) {
        ...
        $.ajax({
            type:'POST',
            url: '{{ route('kriteria_klinik.store') }}',
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
    ...
    function deleteu(id){
        if (confirm("Hapus kriteria klinik ini?") == true) {
            var id = id;
            $.ajax({
                type:"POST",
                url: "{{ url('kriteria_klinik/delete') }}",
                data: { id: id },
                dataType: 'json',
