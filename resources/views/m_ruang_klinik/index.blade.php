    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                ...
                @can('m-ruang-klinik-create')
                    <a class="modal-effect btn btn-primary ms-auto" onClick="add()" href="javascript:void(0)">Tambah Ruang</a>
                @endcan
            </div>
            <div class="card-body">
            ...
            <form action="javascript:void(0)" id="form" name="form" class="form-horizontal" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Nama ruang klinik" id="ruang" name="ruang">
                    </div>
                </div>
...
<script>
    $(function(e) {
        ...
        $('#datatable').DataTable({
            ...
            serverSide: true,
            type: 'JSON',
            ajax: '{{ route('m_ruang_klinik.list') }}',
            columns: [
    ...
    function edit(id){
        $.ajax({
            type:"POST",
            url: '{{ route('m_ruang_klinik.edit') }}',
            data: { id: id },
            dataType: 'json',
            success: function(data){
                $('.modal-title').html("Edit Ruang");
                $('#modal').modal('show');
                $('#id').val(data.id);
    ...
    $('#form').submit(function(e) {
        ...
        $.ajax({
            type:'POST',
            url: '{{ route('m_ruang_klinik.store') }}',
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
            success: (data) => {
                $("#modal").modal('hide');
                $.growl.notice({
                    message: "Berhasil menambahkan ruang."
                });
    ...
    function deleteu(id){
        if (confirm("Hapus ruang klinik ini?") == true) {
            var id = id;
            $.ajax({
                type:"POST",
                url: "{{ url('m_ruang_klinik/delete') }}",
                data: { id: id },
                dataType: 'json',
