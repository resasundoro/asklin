    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">List Data Ruang Klinik</h3>
                @can('ruang-klinik-create')
                    <a class="modal-effect btn btn-primary ms-auto" onClick="add()" href="javascript:void(0)">Tambah Ruang Klinik</a>
                @endcan
            </div>
            <div class="card-body">
        ...
        <div class="modal-content">
            ...
            <form action="javascript:void(0)" id="form" name="form" class="form-horizontal" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        <label class="col-md-3 form-label">Kategori Ruang</label>
                        <div class="col-md-9">
                            <select class="form-control form-select" name="id_ruang_klinik" id="id_ruang_klinik">
                                <option>==Pilih Kategori==</option>
                                @foreach ($m_ruang_klinik as $mrk)
                                    <option value="{{ $mrk->id }}">{{ $mrk->ruang }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row mb-3 grup13">
                        <label class="col-md-3 form-label">Upload Ruang</label>
                        <div class="col-md-9">
...
<script>
    $(function(e) {
        ...
        $('#datatable').DataTable({
            ...
            serverSide: true,
            type: 'JSON',
            ajax: '{{ route('ruang_klinik.list') }}',
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'nama_klinik', name: 'nama_klinik'},
                {data: 'ruangklinik', name: 'ruangklinik'},
                {
                    data: "foto",
                    "render": function (data, type, row) {
                        return "<img src=\"/images/klinik/ruang_klinik/" + row.foto + "\" height=\"50\"/>";
                    },
                },
                {
                    data: 'action',
    ...
    function edit(id){
        $.ajax({
            ...
            success: function(data){
                ...
                $('#id_ruang_klinik').val(data.id_ruang_klinik);
                $('#foto').val(data.foto);
            }
        });
    }
    ...
    $('#form').submit(function(e) {
        ...
        $.ajax({
            ...
            success: (data) => {
                $("#modal").modal('hide');
                $.growl.notice({
                    message: "Berhasil menambahkan ruang klinik."
                });
        ...
        if (confirm("Hapus ruangan ini?") == true) {
            ...
            $.ajax({
                ...
                success: function(res){
                    $('#datatable').dataTable().fnDraw(false);
                    $.growl.notice({
                        message: "Ruangan klinik berhasil dihapus."
                    });
                }
            });
        }
